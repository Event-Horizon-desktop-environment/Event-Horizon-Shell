...
if not present or not (base46._EH_SUPPORT or base46._EH_SUPPORT) then
	vim.notify(
		"base46 plugin not found or incorrect, install a base46 build with Event Horizon support",
		vim.log.levels.ERROR,
		{ title = "eh integration" }
	)
	return
end
...
local settings_file_path = vim.fs.joinpath(config_home, "EventHorizon", "settings.json")
local settings_file = io.open(settings_file_path, "r")
if settings_file == nil then
	vim.notify(
		"cannnot read Event Horizon settings file at '" .. settings_file_path .. "'",
		vim.log.levels.ERROR,
		{ title = "eh integration" }
	)
	return
		...
		_G._matugen_theme_watcher.reload_timer:start(
			...
			vim.schedule_wrap(function()
				...
				if vim.g.colors_name == theme_name then
					...
					vim.notify("Theme reload", vim.log.levels.INFO, { title = "eh integration" })
				end
