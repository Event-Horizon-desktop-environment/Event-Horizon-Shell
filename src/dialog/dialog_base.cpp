...
// Registry listener: grabs the globals the dialog needs (compositor,
// shm, xdg-shell, seat) as they appear.
...
// The dialog's Wayland event loop, running on its own thread.

void DialogBase::thread_main_() {
   
  // Open our own Wayland connection.
  ...
  // And an xdg_toplevel for the window chrome.
  ...
  // Commit the initial state so the compositor sends a configure event.
  ...
  // Allocate the double-buffered SHM buffers.
  for (auto& buf : bufs_) {
  ...
  // Bind the seat and hook up pointer/keyboard listeners.
  ...
  ptr_listener = {
    ...
    .warp = [](void* data, wl_pointer*, wl_fixed_t sx, wl_fixed_t sy) {
      auto* s = static_cast<InputState*>(data);
      s->px = wl_fixed_to_int(sx);
      s->py = wl_fixed_to_int(sy);
      s->dlg->on_pointer_move(s->px, s->py);
  ...
  // Draw the first frame and commit it.
  {
  ...
  // Main loop: dispatch Wayland events, redraw when requested.
  ...
  // Teardown: release input objects, buffers, and the surface hierarchy.
...
// xdg_surface / xdg_toplevel callbacks.
