...
struct WiredIpv4M3State {
  ...
  void applyColours() {
    ...
    apply(routeMetricSlider); apply(requiredTimeoutSlider);
  ...
  void paint(App& app, cairo_t* cr, int contentX, int contentW, double glassOv, int contentTop) {
    ...
    // Card 1: IPv4 Settings
    constexpr int h0 = 52 + 4 * kSliderRowH + kSpacingXL;
    ...
    settings_cat_label(cr, static_cast<double>(cardX + kCardPad), static_cast<double>(cardTop + 15), "IPv4 Settings");

    const int ipv4Band = cardTop + 52;
    paintSlider(ipv4Band + 0 * kSliderRowH, dnsPrioritySlider, 0, -100, 100, "DNS priority");
    paintSlider(ipv4Band + 1 * kSliderRowH, dhcpTimeoutSlider, 30, 0, 120, "DHCP timeout (s)");
    paintToggle(ipv4Band, 2, sendHostname, true, "Send hostname", "Send hostname to DHCP server");
    paintToggle(ipv4Band, 3, ignoreAutoDns, false, "Ignore auto DNS", "Use manual DNS only");

    // Card 2: Advanced
    const int cy1 = cardTop + h0 + kCardGap;
    constexpr int h1 = 52 + 4 * kSliderRowH + kSpacingXL;
    ...
    paintToggle(advBand, 0, neverDefault, false, "Never use as default route", "Do not set as default gateway");
    paintToggle(advBand, 1, mayFail, true, "May fail", "Connection succeeds even if IPv4 fails");
    paintSlider(advBand + 2 * kSliderRowH, routeMetricSlider, 100, 0, 10000, "Route metric");
    paintSlider(advBand + 3 * kSliderRowH, requiredTimeoutSlider, 5000, 0, 60000, "Required timeout (ms)");
  }
...
inline WiredIpv4M3State& wiredIpv4M3() {
  static WiredIpv4M3State s;
  return s;
}
