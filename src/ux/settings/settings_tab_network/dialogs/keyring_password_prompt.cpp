...
static void recalc_hit_areas(double W, double H) {
  ...
  const double btnY = cy + kCardH - pad - 38.0;
...
void keyring_prompt_open(App& app, bool createMode) {
  g_open = true;
  g_createMode = createMode;
  g_password.clear();
  g_confirm.clear();
  g_errorMsg.clear();
  g_hover_button = 0;
  g_press_button = 0;
  g_focus_confirm = false;
  (void)app;
}
...
void keyring_prompt_paint(App& app, cairo_t* cr) {
  ...
  {
    m3::Box box;
    box.setColor(0.0f, 0.0f, 0.0f, 0.2f);
    box.setRadius(20.0f);
  ...
  // Card background
  {
    ...
    if (app.drawChromeMatugen)
      box.setColor(static_cast<float>(app.drawChrome.panelFillR),
  ...
  rr(cr, cx, cy, kCardW, kCardH, 18.0);
  ...
  // Title
  settings_show_text(cr, cx + pad + 40.0, cy + pad + 22.0, g_createMode ? "Set Keyring Password" : "Unlock Keyring", 17.0, 700, Theme::TextR, Theme::TextG, Theme::TextB, 1.0);
  ...
  const int cPrs = (g_press_button == 3) ? 1 : 0;
  {
    m3::Box box;
    if (app.drawChromeMatugen)
      box.setColor(static_cast<float>(app.drawChrome.drawerDimR),
                   static_cast<float>(app.drawChrome.drawerDimG),
  ...
  if (cPrs) {
    {
      m3::Box box;
      box.setColor(0.0f, 0.0f, 0.0f, 0.3f);
      box.setRadius(8.0f);
      box.setGeometry(static_cast<float>(hit_close[0]), static_cast<float>(hit_close[1]),
                      static_cast<float>(hit_close[2]), static_cast<float>(hit_close[3]));
      box.paint(cr);
    ...
    {
      m3::Box box;
      box.setColor(1.0f, 1.0f, 1.0f, 0.2f);
      box.setRadius(8.0f);
      box.setGeometry(static_cast<float>(hit_close[0]), static_cast<float>(hit_close[1]),
                      static_cast<float>(hit_close[2]), static_cast<float>(hit_close[3]));
      box.paint(cr);
  ...
  // Message
  settings_show_text(cr, cx + pad, cy + pad + 48.0, g_createMode ? "Create a password to protect stored secrets."
                                   : "Enter keyring password to unlock stored secrets.", 13.0, 400, Theme::TextR, Theme::TextG, Theme::TextB, 1.0);

  // Error message
  if (!g_errorMsg.empty()) {
    settings_show_text(cr, cx + pad, cy + pad + 70.0, g_errorMsg.c_str(), 11.5, 400, 0.93, 0.38, 0.42, 0.95);
  }
  ...
  cairo_set_source_rgba(cr, Theme::AccR, Theme::AccG, Theme::AccB, g_focus_confirm ? 0.4 : 1.0);
  ...
  if (g_password.empty() && !g_focus_confirm) {
  ...
  double confirmFieldY = 0;
  if (g_createMode) {
    confirmFieldY = fieldY + fieldH + 8.0;
    {
      m3::Box box;
      if (app.drawChromeMatugen)
        box.setColor(static_cast<float>(app.drawChrome.panelFillR),
        ...
        box.setColor(static_cast<float>(Theme::BgR), static_cast<float>(Theme::BgG),
                     static_cast<float>(Theme::BgB), 0.75f);
      box.setRadius(10.0f);
      box.setGeometry(static_cast<float>(cx + pad), static_cast<float>(confirmFieldY),
                      static_cast<float>(fieldW), static_cast<float>(fieldH));
      box.paint(cr);
    }
    rr(cr, cx + pad, confirmFieldY, fieldW, fieldH, 10.0);
    cairo_set_source_rgba(cr, Theme::AccR, Theme::AccG, Theme::AccB, g_focus_confirm ? 1.0 : 0.4);
    cairo_set_line_width(cr, 1.2);
    cairo_stroke(cr);

    std::string confirmShown;
    confirmShown.assign(g_confirm.size(), '*');
    if (confirmShown.size() > 80) confirmShown.resize(77), confirmShown += "...";
    settings_show_text(cr, cx + pad + 8.0, confirmFieldY + 24.0, confirmShown.empty() ? " " : confirmShown.c_str(), 13.0, 400, Theme::TextR, Theme::TextG, Theme::TextB, 1.0);

    if (g_confirm.empty() && g_focus_confirm) {
      settings_show_text(cr, cx + pad + 8.0, confirmFieldY + 12.0, "Confirm password", 11.0, 400, Theme::TextR, Theme::TextG, Theme::TextB, 0.5);
    }
  }

  // Buttons
  const bool hasPw = !g_password.empty();
  const bool confirmOk = !g_createMode || (!g_confirm.empty() && g_password == g_confirm);
  ...
  // Confirm
  {
    ...
    confirmBtn.setLabel(g_createMode ? "Create" : "Unlock");
    ...
    confirmBtn.setEnabled(hasPw && confirmOk);
    confirmBtn.paint(cr);
  }
}
...
void keyring_prompt_consume_pointer_up(App& app) {
  ...
  if (btn == 2 && point_in_rect_d(app.pointerX, app.pointerY, g_hit_confirm)) {
    ...
    if (g_createMode) {
      if (g_password.empty()) {
        g_errorMsg = "Password cannot be empty";
      } else if (g_password != g_confirm) {
        g_errorMsg = "Passwords do not match";
      } else {
        success = eh::keyring::SecretServiceDaemon::instance().create_keyring(g_password);
        if (!success) g_errorMsg = "Failed to create keyring";
        else g_errorMsg.clear();
      }
    } else {
      if (g_password.empty()) {
        g_errorMsg = "Password cannot be empty";
      } else {
        success = eh::keyring::SecretServiceDaemon::instance().unlock_keyring(g_password);
        if (!success) g_errorMsg = "Wrong password";
        else g_errorMsg.clear();
      }
    }
    if (success) {
      keyring_prompt_close(app);
      wifi_password_prompt_retry_pending(app);
    }
    return;
  }
  if (btn == 3) {
...
void keyring_prompt_handle_key(App& app, uint32_t sym, uint32_t state,
  ...
  if (sym == XKB_KEY_Return || sym == XKB_KEY_KP_Enter) {
    if (!g_password.empty()) {
      if (g_createMode && g_password != g_confirm) {
        g_errorMsg = "Passwords do not match";
        return;
      }
      bool success = false;
  ...
  if (sym == XKB_KEY_Tab) {
    if (g_createMode) {
      g_focus_confirm = !g_focus_confirm;
    }
    return;
  }
  if (sym == XKB_KEY_BackSpace) {
    if (g_focus_confirm && g_createMode) {
      if (!g_confirm.empty()) g_confirm.pop_back();
    } else {
  ...
  if (utf8_len > 0) {
    if (g_focus_confirm && g_createMode) {
      g_confirm.append(utf8, static_cast<size_t>(utf8_len));
    } else {
