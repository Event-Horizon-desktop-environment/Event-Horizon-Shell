...
struct HyprlandXwaylandM3State {
  int activeSlider_ = -1;
  Toggle enabled;
  ...
  void applyColours() {
    enabled.setAccentColor(accentR_, accentG_, accentB_);
    enabled.setSurfaceColor(surfaceR_, surfaceG_, surfaceB_);
    enabled.setTextColor(textR_, textG_, textB_);
    enabled.setOutlineColor(outlineR_, outlineG_, outlineB_);
  }
  ...
  void updateHover(const App& app) {
    ...
    enabled.setHovered(enabled.containsPoint(px, py));
  }
  ...
  void paint(App& app, cairo_t* cr, int contentX, int contentW, double glassOv, int contentTop) {
    ...
    // XWayland
    lay.beginGroup(app, cr, glassOv, "XWayland", "X11 compatibility layer", 1, 0,
                   textR_, textG_, textB_);
    hyprland_paint_toggle_card(app, cr, lay, glassOv, enabled, cfg.xwayland.enabled, "Enabled",
                               accentR_, accentG_, accentB_, textR_, textG_, textB_,
                               "Enable XWayland compatibility layer");
    lay.endGroup();
  ...
  bool handlePointerDown(App&, float px, float py) {
    if (enabled.containsPoint(px, py)) { enabled.handlePointerDown(px, py); return true; }
    return false;
  }

  bool handlePointerUp(App& app, float px, float py) {
    if (enabled.pressed()) {
      enabled.handlePointerUp(px, py);
      app.hyprlandConfig.xwayland.enabled = !app.hyprlandConfig.xwayland.enabled;
      return true;
    }
    return false;
  }
  ...
  void handlePointerLeave() { enabled.handlePointerLeave(); }

  void flushSliderValues(App&) const {}
};

inline HyprlandXwaylandM3State& hyprlandXwaylandM3() {
  static HyprlandXwaylandM3State s;
  return s;
}
