...
[[nodiscard]] std::vector<RfkillEntry> listRfkillEntries() {
  ...
  while ((ent = readdir(dir)) != nullptr) {
    ...
    RfkillEntry entry{};
    unsigned value = 0;
    if (!readSysfsUnsigned(base + "index", value)) continue;
    entry.index = value;
    if (!readSysfsUnsigned(base + "type", value)) continue;
    entry.type = value;
    if (readSysfsUnsigned(base + "soft", value)) entry.soft = value != 0;
    if (readSysfsUnsigned(base + "hard", value)) entry.hard = value != 0;
...
[[nodiscard]] std::optional<std::uint32_t> rfkillIndexForNetInterface(std::string_view ifname) {
  ...
  while ((ent = readdir(dir)) != nullptr) {
    ...
    if (name.rfind("rfkill", 0) != 0) continue;
    unsigned value = 0;
    if (!readSysfsUnsigned(phyPath + name + "/index", value)) continue;
    index = value;
    break;
  }
  closedir(dir);
...
[[nodiscard]] RfkillSwitchResult setRfkillSoftBlockedByIndex(std::uint32_t index, bool softBlocked) {
  ...
  if (fd < 0) {
    return {.success = false, .hardBlocked = false, .detail = std::string("cannot open /dev/rfkill: ") + std::strerror(errno)};
  }
  ...
  if (written != static_cast<ssize_t>(sizeof(ev))) {
    return {.success = false, .hardBlocked = false, .detail = written < 0 ? std::string(std::strerror(writeErrno)) : "short write to /dev/rfkill"};
  }
...
RfkillSwitchResult setRfkillSoftBlocked(RfkillDeviceType type, bool softBlocked) {
  ...
  return setRfkillSoftBlockedByIndex(entry->index, softBlocked);
}
