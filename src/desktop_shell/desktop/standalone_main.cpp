...
// Mirror entry_point.cpp / settings_standalone_main.cpp / wallpaper + notifications
...
// identically to the in-process desktop path it replaces. MANDATORY per the
...
static bool eh_desktop_je_pin() {
  size_t n = 0;
  mallctl("narenas", &n, nullptr, nullptr, 0);
  return n > 0;
}

static const bool kEhDesktopJePinned = eh_desktop_je_pin();
#endif

int main() {
  return eh::shell::desktop::run_desktop_standalone();
}
