...
public:
  ...
  void set_disabled_slots(const std::unordered_set<std::string>& slots);
  void set_mpris(eh::mpris::DockMpris* mpris);
  void on_second_tick();
  // Minimum animation cadence currently needed by any widget, in ms (0 = none).
  [[nodiscard]] uint32_t animIntervalMs() const;
...
private:
  ...
  std::unordered_set<std::string> m_disabledSlots;

  [[nodiscard]] bool slot_disabled(const DesktopWidgetConfig& config) const;
};
