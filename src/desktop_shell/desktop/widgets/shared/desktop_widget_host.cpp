...
static std::unique_ptr<DesktopWidget> make_widget(const DesktopWidgetConfig& cfg, eh::mpris::DockMpris* mpris = nullptr) {
  ...
  switch (cfg.type) {
    ...
    case DesktopWidgetType::MediaCompact:
      return std::make_unique<DesktopMediaCompactWidget>(mpris);
    case DesktopWidgetType::WorldClock:
      return std::make_unique<DesktopWorldClockWidget>(cfg.id);
  }
  return nullptr;
}

void DesktopWidgetHost::set_disabled_slots(const std::unordered_set<std::string>& slots) {
  m_disabledSlots = slots;
  m_hoveredWidgetIdx = -1;
}

bool DesktopWidgetHost::slot_disabled(const DesktopWidgetConfig& config) const {
  if (m_disabledSlots.empty()) return false;
  const std::string wid =
      config.widgetId.empty() ? desktop_widget_type_to_widget_id(config.type) : config.widgetId;
  return m_disabledSlots.find(wid) != m_disabledSlots.end();
}
...
void DesktopWidgetHost::set_mpris(eh::mpris::DockMpris* mpris) {
  ...
  for (auto& inst : m_instances) {
    if (inst->widget && (inst->config.type == DesktopWidgetType::MediaPlayer ||
                         inst->config.type == DesktopWidgetType::MediaCompact)) {
...
uint32_t DesktopWidgetHost::animIntervalMs() const {
  uint32_t best = 0;
  for (const auto& inst : m_instances) {
    if (!inst->widget) continue;
    const uint32_t iv = inst->widget->animIntervalMs();
    if (iv != 0 && (best == 0 || iv < best)) best = iv;
  }
  return best;
}
...
void DesktopWidgetHost::paint_all(cairo_t* cr, int layerW, int layerH, const std::string& outputName) {
  ...
  for (auto& inst : m_instances) {
    if (!inst->widget) continue;
    if (slot_disabled(inst->config)) continue;
...
void DesktopWidgetHost::paint_resize_indicators(cairo_t* cr, int resizeIdx, const std::string& outputName) const {
  ...
  if (slot_disabled(inst.config)) return;
