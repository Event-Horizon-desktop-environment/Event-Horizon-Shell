...
std::string desktop_pid_file_path() {
  const std::string d = state_base() + "/event-horizon";
  (void)mkdir(state_base().c_str(), 0755);
  (void)mkdir(d.c_str(), 0755);
  return d + "/desktop.pid";
}

bool desktop_read_pid_file(pid_t* outPid) {
  *outPid = -1;
  FILE* f = fopen(desktop_pid_file_path().c_str(), "rb");
  if (!f) return false;
  char buf[64]{};
...
// Verify the pidfile pid still refers to our horizon-desktop child. Guards the
// kill path against PID reuse: a pid handed back to an unrelated process must
// never be signaled. Matches comm in /proc/<pid>/stat (basename, "horizon-desktop"
// is exactly the 15-char comm limit) or the exe link, so it also recognizes a
// stale child from a previous session whose ppid has been reparented.
bool desktop_pid_is_ours(pid_t pid) {
  ...
  if (n <= 0) return false;
  exe[n] = '\0';
  return std::string_view(exe, static_cast<std::size_t>(n)).find("horizon-desktop") !=
         std::string_view::npos;
}

bool desktop_native_child_active() {
  pid_t p = -1;
  if (!desktop_read_pid_file(&p) || p <= 1) return false;
  if (!desktop_pid_is_ours(p)) {
...
void desktop_write_pid_file(pid_t p) {
  FILE* f = fopen(desktop_pid_file_path().c_str(), "wb");
  if (!f) return;
  fprintf(f, "%d\n", static_cast<int>(p));
  fclose(f);
}
...
void desktop_unlink_pid_file() {
  (void)unlink(desktop_pid_file_path().c_str());
}

pid_t desktop_spawn_native_child() {
  // Reap a stale horizon-desktop child from a previous (possibly killed)
  // session before starting the new one — they'd both own desktop layer
  // surfaces on the same output.
  if (desktop_native_child_active()) {
    pid_t stale = -1;
    if (desktop_read_pid_file(&stale) && stale > 1) desktop_kill_native_child(stale);
  }

  // Resolve the child binary: prefer a `horizon-desktop` sitting next to our own
  // executable (works from a build dir / uninstalled tree), else rely on PATH.
  std::string childBin = "horizon-desktop";
  {
    char self[4096];
    ...
    if (n > 0) {
      ...
      if (::access((dir + "/horizon-desktop").c_str(), X_OK) == 0)
        childBin = dir + "/horizon-desktop";
    }
  }
  ...
  (void)mkdir(state_base().c_str(), 0755);
  (void)mkdir(d.c_str(), 0755);
  const std::string logPath = d + "/horizon-desktop.log";
  ...
  if (rc != 0 || pid < 0) {
    std::cerr << "[desktop] spawn horizon-desktop failed: " << strerror(rc) << "\n";
    return -1;
  }
  std::cout << "[desktop] spawned horizon-desktop pid=" << static_cast<int>(pid)
            << " (log: " << logPath << ")\n";
  return pid;
}
