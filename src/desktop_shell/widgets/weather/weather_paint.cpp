...
struct WeatherTextCache {
  ...
  double tempFontPx = 0;
  double unitFontPx = 0;
  PangoLayout* tempLayout = nullptr;
  PangoLayout* unitLayout = nullptr;
  ~WeatherTextCache() {
    if (tempLayout) g_object_unref(tempLayout);
    if (unitLayout) g_object_unref(unitLayout);
  }
  ...
  WeatherTextCache(WeatherTextCache&& o) noexcept
      : tempStr(std::move(o.tempStr)), unit(std::move(o.unit)),
        ...
        tempFontPx(o.tempFontPx), unitFontPx(o.unitFontPx),
        tempLayout(o.tempLayout), unitLayout(o.unitLayout) {
    o.tempLayout = nullptr;
    o.unitLayout = nullptr;
  }
  WeatherTextCache& operator=(WeatherTextCache&& o) noexcept {
    ...
    unitW = o.unitW;
    unitH = o.unitH;
    tempFontPx = o.tempFontPx;
    unitFontPx = o.unitFontPx;
    tempLayout = o.tempLayout;
    unitLayout = o.unitLayout;
    o.tempLayout = nullptr;
    o.unitLayout = nullptr;
    return *this;
  }
...
void weather_layout_inner(cairo_t* measure_cr, std::string_view instance_id, double icon_ref_px,
                          const ControlCenterWeatherState& ws, double& out_icon_px, double& out_gap1,
...
double dock_weather_slot_width(cairo_t* measure_cr, const eh::config::ShellConfig& sc, std::string_view instance_id,
  ...
  weather_layout_inner(measure_cr, instance_id, icon_ref_px, ws, iconPx, gap1, tempW, pillW, pillH);
  (void)pillH;
...
void paint_weather_slot(cairo_t* cr, const eh::config::ShellConfig& sc, std::string_view instance_id, double x, double y,
  ...
  const ControlCenterWeatherState ws = control_center_weather_state(sc, instance_id);
  double iconPx{}, gap1{}, tempW{}, pillW{}, pillH{};
  weather_layout_inner(nullptr, instance_id, icon_ref_px, ws, iconPx, gap1, tempW, pillW, pillH);
  ...
  const double gAlpha = ws.available ? 0.92 : 0.45;
  eh::shell::draw_material_glyph(cr, baseX + iconPx * 0.5, cy, iconPx, glyph, mc.accentR, mc.accentG,
                                  mc.accentB, gAlpha);
  ...
  if (!wEntry.tempLayout || wEntry.tempFontPx != fontPx) {
    if (wEntry.tempLayout) g_object_unref(wEntry.tempLayout);
    wEntry.tempLayout = make_layout(cr, fd.c_str());
    wEntry.tempFontPx = fontPx;
  } else {
    pango_cairo_update_layout(cr, wEntry.tempLayout);
  }
  PangoLayout* layout = wEntry.tempLayout;
  const std::string tempStr = weather_temp_display(ws);
  pango_layout_set_text(layout, tempStr.c_str(), -1);
  ...
  if (!wEntry.unitLayout || wEntry.unitFontPx != pillFont) {
    if (wEntry.unitLayout) g_object_unref(wEntry.unitLayout);
    wEntry.unitLayout = make_layout(cr, fdU.c_str());
    wEntry.unitFontPx = pillFont;
  } else {
    pango_cairo_update_layout(cr, wEntry.unitLayout);
  }
  PangoLayout* layU = wEntry.unitLayout;
