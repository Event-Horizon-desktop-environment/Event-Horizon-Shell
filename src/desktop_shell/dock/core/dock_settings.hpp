...
// Extra vertical space reserved above the dock for tiled/maximized windows.
// The layer surface's exclusive zone is dockHeight + this gap, so windows rest
// a configurable distance above the dock's top edge.
[[nodiscard]] inline int dock_exclusive_zone_gap_px(const DockSettings& s) {
  return std::max(0, std::min(100, s.dockExclusiveZoneGap));
}
