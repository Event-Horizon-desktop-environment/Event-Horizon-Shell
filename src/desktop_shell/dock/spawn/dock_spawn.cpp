...
bool dock_read_pid_file(pid_t* outPid) {
  *outPid = -1;
  FILE* f = fopen(dock_pid_file_path().c_str(), "rb");
  if (!f) return false;
  char buf[64]{};
...
// Verify the pidfile pid still refers to our horizon-dock child. Guards the
...
// has been reparented.
bool dock_pid_is_ours(pid_t pid) {
  ...
  if (n <= 0) return false;
  exe[n] = '\0';
  return std::string_view(exe, static_cast<std::size_t>(n)).find("horizon-dock") !=
         std::string_view::npos;
}

bool dock_native_child_active() {
  pid_t p = -1;
  if (!dock_read_pid_file(&p) || p <= 1) return false;
  if (!dock_pid_is_ours(p)) {
...
void dock_write_pid_file(pid_t p) {
  FILE* f = fopen(dock_pid_file_path().c_str(), "wb");
  if (!f) return;
  fprintf(f, "%d\n", static_cast<int>(p));
  fclose(f);
}
...
void dock_unlink_pid_file() {
  (void)unlink(dock_pid_file_path().c_str());
}

pid_t dock_spawn_native_child() {
  // Reap a stale horizon-dock child from a previous (possibly killed) session
  // before starting the new one — they'd both own dock layer surfaces.
  if (dock_native_child_active()) {
    pid_t stale = -1;
    if (dock_read_pid_file(&stale) && stale > 1) dock_kill_native_child(stale);
  }

  // Resolve the child binary: prefer a `horizon-dock` sitting next to our own
  // executable (works from a build dir / uninstalled tree), else rely on PATH.
  std::string childBin = "horizon-dock";
  {
    char self[4096];
    ...
    if (n > 0) {
      ...
      if (::access((dir + "/horizon-dock").c_str(), X_OK) == 0)
        childBin = dir + "/horizon-dock";
    }
  }
  ...
  (void)mkdir(state_base().c_str(), 0755);
  (void)mkdir(d.c_str(), 0755);
  const std::string logPath = d + "/horizon-dock.log";
  ...
  if (rc != 0 || pid < 0) {
    std::cerr << "[dock] spawn horizon-dock failed: " << strerror(rc) << "\n";
    return -1;
  }
  std::cout << "[dock] spawned horizon-dock pid=" << static_cast<int>(pid)
            << " (log: " << logPath << ")\n";
  return pid;
}
